class Solution:
    def minScoreTriangulation(self, values: List[int]) -> int:
        ...
        n = len(values)
        dp = [[0] * n for _ in range(n)]  # dp[i][j] will store the minimum triangulation score for polygon between i and j
        ...
        for length in range(3, n + 1):  # polygon must have at least 3 vertices
            for i in range(n - length + 1):
                j = i + length - 1  # j is the endpoint of current polygon segment
                dp[i][j] = float('inf')
